...
"""
PHOENIX REAL v5.9.1 - ESTRATÉGIA HÍBRIDA (TENDÊNCIA + RANGE)
- CORREÇÕES APLICADAS:
...
class PhoenixEngine:
    ...
    def _check_entry_range(self, price, time_ms, close, high, low, volume, rsi):
        # CORREÇÃO: adicionar verificações de cooldown, pausa e circuit breaker
        if self.cooldown > 0:
            return False
        with self.state_lock:
            if self.shared_state.get("operating_paused", False) or self.shared_state.get("circuit_breaker", False):
    ...
    while True:
        if not menu_ativo:
            ...
            with config_lock:
                ...
                exit_threshold = s.get('exit_score_threshold', 8.5)
            ...
            print(f"{Fore.CYAN}🐦‍🔥 PHOENIX REAL v5.9.1 (HÍBRIDO) {Fore.WHITE}| UPTIME: {uptime}")
            ...
            if s["em_operacao"]:
                ...
                if "entry_score" in s:
                    ...
                    print(f"  SCORE ENTRADA: {entry_score_color}{s['entry_score']:.2f}{Fore.WHITE} / {entry_threshold:.2f}")
                if "exit_score" in s:
                    exit_score_color = Fore.RED if s["exit_score"] >= exit_threshold else Fore.GREEN
                    print(f"  SCORE SAÍDA  : {exit_score_color}{s['exit_score']:.1f}{Fore.WHITE} / {exit_threshold:.1f}")
            print(f"{Fore.YELLOW}{'='*80}")
...
def acionar_menu(signum, frame):
    ...
    print(f"{Fore.MAGENTA}║    MENU DE CONTROLE PHOENIX v5.9.1   ║")
    print(f"{Fore.MAGENTA}╠══════════════════════════════════════╣")
